...
public static int updateFees(StudentBean bean){
	int status=0;
	try{
		Connection con=DB.getCon();
		PreparedStatement ps=con.prepareStatement("update fee_student set  fee=?,paid=?,due=? where rollno=?");
		ps.setInt(1,bean.getFee());
		ps.setInt(2,bean.getPaid());
		ps.setInt(3,bean.getDue());
